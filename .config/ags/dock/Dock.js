    ...
    children: options.desktop.dock.pinned_apps.bind('value').transform(v => v
        ...
        .map(({ app, term }) => AppButton({
            ...
            setup: button => button.hook(Hyprland, () => {
                ...
                const icon = button.child.child.child;
                button.on('button-release-event', () => icon.toggleClassName('clicked', true))
                // on startup, for all icons, stop animation (1 iteration) and make the bar last 2 secs
            }),
        })),
    ),
});
